...
import { EmptyDrawer, Kao } from "@/components/archive-ui";
...
function MemoryBox() {
  ...
  return (
    <main className="px-4 pt-8 sm:px-6">
      ...
      <div className="card-object mt-5 overflow-hidden">
        ...
        <div className="grid place-items-center gap-4 px-4 py-8">
          <Kao face={open ? "ヽ(°〇°)ﾉ" : "(¬_¬ )"} className="text-base" />
          <button
            onClick={surprise}
      ...
      <div className="mt-4">
        {items.length === 0 ? (
          ...
          <Link
            ...
            params={{ id: picked.id }}
            key={picked.id}
            className="folder anim-box block p-4"
          >
            <span className="folder-tab-label">{catMeta(picked.category).plural}</span>
            <Kao face={catMeta(picked.category).emoji} className="text-[0.7rem]" />
